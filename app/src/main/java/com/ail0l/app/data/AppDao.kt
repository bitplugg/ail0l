...
import com.ail0l.app.data.entities.FactEmbeddingEntity
...
import com.ail0l.app.data.entities.PersonaEntity
...
interface AppDao {
    ...
    @Query("SELECT * FROM facts ORDER BY favorite DESC, createdAt DESC LIMIT :limit")
    ...
    suspend fun upsertEmbedding(e: FactEmbeddingEntity)
    ...
    // Personas
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertPersona(p: PersonaEntity): Long

    @Query("SELECT * FROM personas ORDER BY name")
    fun observePersonas(): Flow<List<PersonaEntity>>

    @Query("SELECT * FROM personas WHERE enabled = 1 ORDER BY name")
    fun observeEnabledPersonas(): Flow<List<PersonaEntity>>

    @Query("SELECT * FROM personas WHERE id = :id LIMIT 1")
    suspend fun persona(id: Long): PersonaEntity?

    @Query("SELECT * FROM personas WHERE enabled = 1 ORDER BY id LIMIT 1")
    suspend fun activePersona(): PersonaEntity?

    @Query("UPDATE personas SET enabled = CASE WHEN id = :id THEN 1 ELSE 0 END")
