#!/usr/bin/env bash
#
# AIL0L · сборка, версионирование и установка APK на устройство.
#
#   ./deploy.sh            — собрать debug, спросить про версию, поставить на устройство
#   ./deploy.sh release    — собрать release (подписан debug-ключом без keystore.properties)
#   ./deploy.sh --no-install — собрать, но НЕ ставить на устройство
#   ./deploy.sh --no-version — не менять versionName (просто build и install)
#
# Что делает:
#   1. Читает текущий versionName из app/build.gradle.kts.
#   2. Спрашивает, добавить ли суффикс: (-beta / -alpha / -rc / свой вариант / без суффикса).
#   3. Собирает APK через ./gradle.sh.
#   4. Показывает версию, размер и путь к APK.
#   5. Если устройство подключено (adb) — устанавливает APK.
...
echo "──────────────────────────────────────────────"
echo "  AIL0L » сборка и установка на устройство"
echo "──────────────────────────────────────────────"
...
# ---------------------------------------------------------------------------
# 2. Версия (спрашиваем суффикс к каждому новому билду)
# ---------------------------------------------------------------------------
...
if [[ "$SUFFIX" == "auto" ]]; then
    while true; do
        ...
        case "${ANSWER,,}" in
            ...
            *)
                # свой суффикс: вводится как есть, например "beta.2"
